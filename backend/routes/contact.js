...
/**
 * POST /api/contact
 * Receives and validates contact form submission, then dispatches email via SMTP
 */
router.post("/", async (req, res) => {
  try {
    const rawData = req.body || {};

    // 1. Trim and normalize input values
    ...
    // 3. Process attachments (e.g. uploaded resume / documents)
    ...
    // 4. Return clean 200 success response
    return res.status(200).json({
      success: true,
      message: "Your message has been sent successfully.",
    });
  } catch (error) {
    // Log detailed internal server error only on console (never expose to client)
    console.error("❌ [Contact Route Error] Failed to process contact submission:", error);
